...
public function add_post($receiver_name,$sender,$house,$street,$area,$city,$pincode,$mail_type,$weight,$ack,$priority,$coll_address,$coll_time,$dispatch_date,$basic_charge,$delivery_charge,$ack_charges,$total_charges,$coupon_id,$discount,$total_payment,$collection_date)
{
    ...
    while($dboy=mysqli_fetch_array($Resp_dboy))
    {
      ...
      // send notification to dboy
        ...
        echo $resp=send_notification_android($not, $reg_ids_android, $title, $body);
...
{
    $stmt = $this->con->prepare("select concat(ca.address_label,ca.house_no,ca.street,c1.city_name,a1.area_name) from customer_address ca, area a1,city c1 where ca.area_id=a1.aid and ca.city_id=c1.city_id and ca.cust_id=?");
    $stmt->bind_param("i",$sender_id);
    $stmt->execute();
    $result = $stmt->get_result();
    $stmt->close();
...
{
    $stmt = $this->con->prepare("SELECT ca.*,a1.area_name,c1.city_name FROM `customer_address` ca,city c1,area a1 where ca.area_id=a1.aid and ca.city_id=c1.city_id and  ca.cust_id=?");
    $stmt->bind_param("i",$cid);
    $stmt->execute();
    $result = $stmt->get_result();
    $stmt->close();
