...
require_once '../include2/DbOperation.php';
...
$app->post('/job_action', function () use ($app) {
    ...
    $mail_type_id=isset($data->mail_type_id)?$data->mail_type_id:"1";
    ...
    //get weight range from weight
   // $weight_range=$db->get_weight_range($weight,$mail_type_id);
    if ($db->job_action($job_id, $status)) {
        ...
        {   
            ...
            while ($token_ios = mysqli_fetch_array($res_token)) {
                $reg_ids_ios[$inc2++] = $token_ios["device_token"];
            }
...
/*
*name:get weight range list
*param:mail_type
method:post
*/
$app->post('/get_weight_range_list', function () use ($app) {
    ...
    $data["data"] = array();
    $response = array();
    $weight_data=$db->get_weight_data($weight_id);
    $weight_from=$weight_data["gm_from"];
    $weightdata = $db->get_weight_range_list($mail_type_id,$weight_from);
    
    if (!empty($weightdata)) {
        $data['result'] = true;
        $data['message'] = "";
        
        while ($row = $weightdata->fetch_assoc()) {
            $temp= new stdClass();
            foreach ($row as $key => $value) {
                $temp->$key= strval($value);
